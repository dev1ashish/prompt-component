...
import { EditorContext as EditorContextImport } from "../context/editorContext"
...
const ModelCard = forwardRef((props, ref) => {
  ...
  }, [props.showProbabilities, props.showHighlights])
  ...
  const getEditorState = useCallback((): EditorState => props.editorState, [props.editorState])
  ...
  return (
    ...
    <div className="flex-1 flex border border-slate-400">
      <div className={`relative overflow-hidden flex-1 flex flex-col p-2`}>
        ...
        <div className="absolute bottom-[.5em] right-[1em] z-[2] flex gap-2">
          {!generating && (
...
const ModelsCompletion = ({showDialog}) => {
  ...
  const handleClearAll = () => {
    Object.values(modelEditorRefs.current).forEach((ref: any) => {
      if (ref && ref.reset) ref.reset()
    })
    setModelsCompletionState({})
  ...
  return (
    <div className="flex flex-col">
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-lg font-semibold">Model Outputs</h3>
        <Button 
          variant="destructive" 
      ...
      <TransitionGroup className="grid h-full mt-3 pr-1 overflow-auto grid-cols-1 gap-2 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 3xl:grid-cols-4 5xl:grid-cols-5 6xl:grid-cols-6 8xl:grid-cols-8">
        {modelsStateContext
